...
const StepUpStepDownContent = ({ userAnalysis, dispatchUserAnalysis, isLoading: isAnalysisLoading }: Props) => {
  ...
  const [belowCustomerToBeAdded, setBelowCustomerToBeAdded] = useState<string>("");

  const { mutate: updateUserAnalysis, isLoading: isUpdatingUserAnalysis } = useMutation(
    (userAnalysis: IUserAnalysis) => {
  ...
  return (
    <section className="form-container">
      ...
      {isAnalysisLoading && <Spinner className="flex items-center px-1 text-2xl" message="Loading customers..." />}
      {!isAnalysisLoading && (
        <form className="flex flex-col gap-4">
          <div className="flex flex-col gap-4 p-5 bg-dark-50 rounded-2xl">
            <h4 className="text-dark-400 text-[1.75rem] font-hero-semibold">10% above</h4>
            <div className="pill-yellow-50 p-3">
              <div className="w-[3rem] h-[3rem]">
                <Image src="/bulb.svg" alt="Bulb Icon" width={0} height={0} className="w-full h-auto" />
              </div>
              <p className="text-xl text-dark-300">Who are your customers if you step up 10% above?</p>
            </div>
            <ul className="flex flex-col gap-4">
              {!userAnalysis.above?.length && !isAnalysisLoading && (
                <div className="w-full flex justify-start items-center">
                  <p className="p-5 text-dark-400 text-xl text-center italic">Start adding step up customers...</p>
                </div>
              )}
              <div className="flex flex-wrap items-center gap-4">
                ...
                <button
                  ...
                  className={
                    !!aboveCustomerToBeAdded ? "btn-primary" : "btn-primary hover:bg-primary-300 cursor-not-allowed"
                  }>
                  <FontAwesomeIcon className="w-3 h-auto cursor-pointer hover:text-gray-600" icon={faPlus} />
                  Add more
                </button>
              </div>
              {!!userAnalysis.above?.length &&
                userAnalysis.above.map((item, index) => (
                  <li key={index} className="relative xl:w-[69%] flex items-center">
                    <input type="text" className="dark-input text-ellipsis" value={item} readOnly />
                    <button
                      type="button"
                      onClick={() => {
                        userAnalysis.above = userAnalysis.above.filter((item, i) => index !== i);
                        dispatchUserAnalysis((prevValue: IUserAnalysis) => {
                          return {
                            ...prevValue,
                          };
                        });
                      ...
                      <FontAwesomeIcon icon={faTimes} className="w-4 text-dark-300 hover:text-dark-400" />
                    </button>
                  </li>
                ))}
            </ul>
          ...
          <div className="flex flex-col gap-4 p-4 bg-dark-50 rounded-2xl">
            <h4 className="text-dark-400 text-[1.75rem] font-hero-semibold">Your Customers</h4>
            <div className="pill-yellow-50 p-3">
              <div className="w-[3rem] h-[3rem]">
                <Image src="/bulb.svg" alt="Bulb Icon" width={0} height={0} className="w-full h-auto" />
              </div>
              <p className="text-xl text-dark-300">Who are your current customers?</p>
            </div>
            {!userAnalysis.customers?.length && !isAnalysisLoading && (
              <div className="w-full flex justify-start items-center">
                <p className="p-5 text-dark-400 text-xl text-center italic">Start adding current customers...</p>
              </div>
            )}
            <div className="flex flex-row flex-wrap items-center gap-4">
              ...
              <button
                type="button"
                onClick={() => {
                  userAnalysis.customers.push(currentCustomerToBeAdded);
                  dispatchUserAnalysis({
                    ...userAnalysis,
                  });
                ...
                className={
                  !!currentCustomerToBeAdded ? "btn-primary" : "btn-primary hover:bg-primary-300 cursor-not-allowed"
                }>
                <FontAwesomeIcon className="w-3 h-auto cursor-pointer hover:text-gray-600" icon={faPlus} />
                Add more
              </button>
            ...
            <ul className="flex flex-col gap-4">
              {!!userAnalysis.customers?.length &&
                userAnalysis.customers.map((item, index) => (
                  <li key={index} className="relative xl:w-[69%] flex items-center">
                    <input type="text" className="dark-input" value={item} readOnly />
                    <button
                      type="button"
                      onClick={() => {
                        userAnalysis.customers = userAnalysis.customers.filter((item, i) => index !== i);
                        dispatchUserAnalysis((prevValue: IUserAnalysis) => {
                          return {
                            ...prevValue,
                          };
          ...
          <div className="flex flex-col gap-4 p-5 bg-dark-50 rounded-2xl">
            <h4 className="text-dark-400 text-[1.75rem] font-hero-semibold">10% below</h4>
            <div className="pill-yellow-50 p-3">
              <div className="w-[3rem] h-[3rem]">
                <Image src="/bulb.svg" alt="Bulb Icon" width={0} height={0} className="w-full h-auto" />
              </div>
              <p className="text-xl text-dark-300">Who are your customers if you step down 10% below?</p>
            </div>
            {!userAnalysis.below?.length && !isAnalysisLoading && (
              <div className="w-full flex justify-start items-center">
                <p className="p-5 text-dark-400 text-xl text-center italic">Start adding step down customers...</p>
              </div>
            )}
            <div className="flex flex-row flex-wrap items-center gap-4">
              <input
              ...
              <button
                ...
                className={
                  !!belowCustomerToBeAdded ? "btn-primary" : "btn-primary hover:bg-primary-300 cursor-not-allowed"
                }>
                <FontAwesomeIcon className="w-3 h-auto cursor-pointer hover:text-gray-600" icon={faPlus} />
                Add more
              </button>
                ...
                userAnalysis.below.map((item, index) => (
                  <li key={index} className="relative xl:w-[69%] flex items-center">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        userAnalysis.below = userAnalysis.below.filter((item, i) => index !== i);
                        dispatchUserAnalysis((prevValue: IUserAnalysis) => {
                          return {
                            ...prevValue,
                          };
            ...
            {!!userAnalysis.id && (
              <div
                ...
                <span className="text-xl text-md text-white">
                  Go to next - <span className="text-white">Road Map </span>
                </span>
              </div>
            )}
          </div>
