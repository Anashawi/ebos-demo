...
const IdeasModal: NextPage<Props> = ({ isOpen, toggle }) => {
  ...
  useEffect(() => {
    if (userIdeas) {
      setIdeaFactors(userIdeas.ideas);
      setUserIdeasId(userIdeas.id);
    }
  }, [userIdeas]);
  ...
  return (
    <>
      <Modal
        config={{
          className: "p-5 lg:p-10 w-2/3 min-w-[320px] max-w-[850px] max-h-[650px]",
          isShown: isOpen,
        ...
        <div className="flex flex-col gap-7">
          <h2 className="flex gap-5 items-center text-4xl text-gray-700 mb-5">
            ...
            {(isCreatingIdea || isDeletingIdea || isUpdatingIdea) && (
              <Spinner className="text-lg items-center font-normal" message="Saving Ideas ..." />
            )}
          </h2>
        </div>
        <div className="h-[85%] overflow-auto">
          <div className="relative flex-auto py-3">
            {isUserIdeasLoading && <Spinner className="mb-10 text-lg" message="Loading ideas ..."></Spinner>}
            {!ideaFactors?.length && !isUserIdeasLoading && (
              <div className="w-full flex items-center">
                <p className="mb-10 text-xl text-center italic">Start adding your ideas...</p>
              </div>
            )}
            {!!ideaFactors?.length && (
              <ul className="flex flex-col gap-2 mb-5 max-h-[350px] overflow-auto">
                {ideaFactors.map((idea: IIdea, index: number) => (
                  <li key={index} className="flex gap-5 justify-between items-center text-dark-400 border-t">
                    <div className="flex gap-5">
                      <span className="text-xl">{idea.name}</span>
                      <span>-</span>
                      <span className="text-xl">{idea.ownerName}</span>
                    </div>
                    <button
                      onClick={() => {
                ...
                <Form>
                  <div className="flex flex-wrap gap-5 items-start">
                    <div className="grow flex gap-5">
                      <div className="grow">
                        <Field type="text" className="w-full light-input" placeholder="New idea" name="name" />
                        <ErrorMessage name={`name`}>
                          {(msg) => <div className="text-lg text-rose-500">{msg}</div>}
                        </ErrorMessage>
                      </div>
                      <div className="grow max-w-[15rem]">
                        <Field type="text" className="w-full light-input" placeholder="idea owner" name="ownerName" />
                    ...
                    <div className="grow lg:grow-0 flex justify-end text-xl">
                      <button
                        type="submit"
                        className={isSubmitting ? "btn-rev btn-disabled" : "btn-rev"}
