...
const ChartsContent = ({ videoPropName, videoLabel, chartProducts, isChartDataLoading, setOpenaiMessage }: Props) => {
  ...
  const isNotLoadingWithChartData = !isChartDataLoading && chartProducts.length > 0;

  return (
    <>
      <article className="helpers-modals-and-charts">
        <ChartsButton title="Watch Video Tutorial" icon={undefined} clickCallback={() => toggleVideoModal(true)} />
        ...
        {videoPropName === videoPropNamesEnum.roadMap && (
          <Link href="/org/report" className="btn-primary-light text-center text-white hover:text-white">
            Generate Report
          </Link>
        )}
        {isChartDataLoading && <Spinner message="Loading products chart..." className="p-5 items-center text-xl" />}
        {isNotLoadingWithChartData &&
          ...
          chartProducts.map((product, index) => (
            <div key={index} className="h-[300px]">
              ...
              {videoPropName === videoPropNamesEnum.redOcean && <RedOceanProductChart product={product} />}
              {videoPropName === videoPropNamesEnum.blueOcean && <BlueOceanProductChart product={product} />}
            </div>
          ))}
        {isNotLoadingWithChartData && videoPropName === videoPropNamesEnum.products && (
          <Chart {...chart} legendToggle />
        )}
        {isNotLoadingWithChartData && videoPropName === videoPropNamesEnum.nonCustomers && (
          <NonCustomersReview userNonCustomers={chartProducts[0]} />
        )}
        {isNotLoadingWithChartData && videoPropName === videoPropNamesEnum.stepUpStepDownModel && (
          <StepUpStepDownCustomersReview userAnalysis={chartProducts[0]} isLoading={isNotLoadingWithChartData} />
        )}
      </article>
      {/* ideas modal */}
      <IdeasModal
      ...
      <Modal
        config={{
          ...
          className: "flex flex-col lg:w-1/3 max-w-[1320px] rounded-xl overflow-hidden p-5 lg:p-10",
        }}>
        <SharedVideoForm
