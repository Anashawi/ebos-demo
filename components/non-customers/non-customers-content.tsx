        ...
        {!areNonCustomersLoading && (
          <form className="flex flex-col gap-4">
            <section className="flex flex-col gap-8 p-5 bg-dark-50 rounded-2xl">
              <h3 className="text-[1.75rem] text-dark-400 font-hero-semibold">Soon to be non-customers</h3>
              ...
              <ul className="flex flex-col gap-4">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <button
                    ...
                    className={
                      !!nonCustomerToBeAdded ? "btn-primary" : "btn-primary hover:bg-primary-300 cursor-not-allowed"
                    }>
                  ...
                  userNonCustomers.soonNonCustomers.map((nonCustomer, index) => (
                    <li key={index} className="relative xl:w-[69%] flex items-center">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          userNonCustomers.soonNonCustomers = userNonCustomers.soonNonCustomers.filter(
                            (nonC, i) => i !== index
                          );
                          dispatchUserNonCustomers({
                            ...userNonCustomers,
              ...
              <ul className="flex flex-col gap-4">
                ...
                {!!userNonCustomers.refusingNonCustomers?.length &&
                  userNonCustomers.refusingNonCustomers.map((refusingCustomer, index) => (
                    <li key={index} className="relative xl:w-[69%] flex items-center">
                      <input type="text" className="dark-input" value={refusingCustomer} readOnly />
                      <button
                        type="button"
                        onClick={() => {
                          userNonCustomers.refusingNonCustomers = userNonCustomers.refusingNonCustomers.filter(
                            (nonC, i) => i !== index
                          );
                          dispatchUserNonCustomers({
                            ...userNonCustomers,
              ...
              <ul className="flex flex-col gap-4">
                ...
                {!!userNonCustomers.unwantedNonCustomers?.length &&
                  userNonCustomers.unwantedNonCustomers.map((unwantedCustomer, index) => (
                    <li key={index} className="relative xl:w-[69%] flex items-center">
                      <input type="text" className="dark-input" value={unwantedCustomer} readOnly />
                      <button
                        type="button"
                        onClick={() => {
                          userNonCustomers.unwantedNonCustomers = userNonCustomers.unwantedNonCustomers.filter(
                            (nonC, i) => i !== index
                          );
                          dispatchUserNonCustomers({
                            ...userNonCustomers,
