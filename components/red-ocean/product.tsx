...
const RedOceanProduct: NextPage<Props> = ({ product, index }) => {
  ...
  function trimText(text: string, maxLength: number, suffix: string = "..."): string {
            ...
            <ul className="flex flex-col gap-2 overflow-auto pb-10">
              {product.factors && product.factors.length > 0
                ? product.factors.map((factor, factorIndex) => (
                    <li key={`fact-${factorIndex}`} className="flex gap-5 justify-start items-start">
                      ...
                      {product.competitors?.map((comp, compIndex) => (
                        <Fragment key={comp.uuid}>
                          {!comp.isUntapped && (
                            <div className="flex flex-col">
                              <label className="w-full">{compIndex === 0 ? `Me` : trimText(comp.name, 18)}</label>
                              <Field
