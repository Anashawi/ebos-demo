...
const useFuturesChart = (products: IProduct[]) => {
    ...
    const updateChartProps = () => {
        let ticks: any = products
            .map(prod =>
                prod.futures?.map(future => {
                    return future.year;
                })
            )
            .flat()
            ...
            .map(
                (prod, prodIndex: number) =>
                    [...(prod.futures ?? [])]
                        ...
                        .map(future => {
                            const bubbleXPos: number = ticks.findIndex(
                                (t: any) => +t.f === future.year
                            );
                            return [
                                "",
                                bubbleXPos,
        ...
        chart.data = [
            ["Product Name", "Year", "Level", "Product Name", "Sales"],
            ...rows,
        ];
