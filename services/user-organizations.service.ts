...
export async function getOne(currentUserId: string) {
   try {
      await dbConnect();
      const result = await UserOrganizations.findOne({ userId: currentUserId });
      if (!result) {
         throw new Error("User organizations are not found");
      }
...
export async function insertOne(userOrganizations: IUserOrganizations) {
   try {
      await dbConnect();
      const frontEndUserOrganizations = new UserOrganizations(userOrganizations)
...
export async function updateOne(userOrganizations: IUserOrganizations) {
   try {
      ...
      const updatedUserOrganization = await UserOrganizations.findById(userOrganizations.id);
